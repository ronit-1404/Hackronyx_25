...
// 1. Parse your `content.txt`** into `recommendationsArray` (can be done in backend or JS).
// 2. Call your APIs** (video, audio, screen) and aggregate their outputs as shown.
// 3. Store user preference** at registration/login.
// 4. Call `runSentimentCoach(outputs, userPreference, recommendationsArray)`** after each new model output.
// 5. The popup will appear** in your UI with the right suggestion or encouragement.
// 6. For summaries, you can call Gemini API with the screen analyzer output and show the result in the popup.
